...
def get_processor_choices(request: HttpRequest, mode: str | None = None) -> list[Tuple[str, str]]:
    result = []
        ...
        if processor_mode == 'default' or processor_mode == mode:
            for processor_path, klass in processor_paths.items():
                ...
                if processor.is_available(request):
                    result.append((processor_path, processor.TITLE or processor.__class__.__name__))
    return result

def get_processor_class(request: HttpRequest, requested_processor_path: str) -> ServerSideContentProcessor | None:
